...
* DI(의존성 주입)
  ...
  * 의존성 주입 방식
    ...
    * **Constructor based Injection** : 생성자를 통해 의존 관계를 주입하는 방법. 주입받는 객체가 변하지 않도록 강제할 수 있으며, 의존성의 순환 참조에 대한 예방이 가능하다.
      > 순환 참조 : A가 B에 의존하고 B가 A에 의존하는 형태
...
### 인덱스
* 인덱스는 데이터의 검색 성능을 향상시켜주는 자료 구조이다.
* 인덱스 원리
...
### 세션 & 쿠키
* 세션과 쿠키는 일반적으로 HTTP의 비연결성(통신 이후 연결을 끊음)과 비상태성(통신 상태를 유지하지 못함)을 보완하기 위한 기술이다.
* 세션
  * 정보를 서버에 저장한다.
